	...
	update(time){
		...
		for(let k=0;k<this.iteration;k++){
			...
			for(let i of this.constraint){
				...
				i.b.force.add_in(new vec2(0,0).set(ff/i.b.mass,dis.deg()))
			}
			//重力及阻力
...
function collision_rect_rect(A,B){
	...
	if(Math.abs(pAB.x)*2<A.scale.x+B.scale.x&&Math.abs(pAB.y)*2<A.scale.y+B.scale.y){
		...
		if(A.mass===Infinity&&B.mass===Infinity){
			vCm=A.velocity.add(B.velocity).scale(1/2)
		}else if(A.mass===Infinity){
			vCm=A.velocity.copy()
		}else if(B.mass===Infinity){
			vCm=B.velocity.copy()
		}else{
			vCm=A.velocity.scale(A.mass).add(B.velocity.scale(B.mass)).scale(1/(A.mass+B.mass))
		}


		let vCmA=A.velocity.minus(vCm)
		let vCmB=B.velocity.minus(vCm)
		...
		if(border_dis_x<border_dis_y){
			...
			if(Math.sign(pAB.x)===-Math.sign(vAB.x)){
				//一維完全彈性碰撞
				A.velocity.x=vCm.x-vCmA.x*0
				B.velocity.x=vCm.x-vCmB.x*0
				//callback
